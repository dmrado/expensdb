...
<form action="datepickerin.php" method="post">
...
if(isset($_POST['data_from']) || isset($_POST['data_to'])){
    ...
    try {
        ...
        $query = "SELECT * FROM  incdescrtb";

        if(isset($_POST['data_from']) && $_POST['data_from'] !== '') $query .= " WHERE incdate >= '".$_POST['data_from']."' AND userid = '".$userid."'";

        if(isset($_POST['data_to']) && $_POST['data_to'] !== '') $query .= " AND incdate <= '".$_POST['data_to']."' AND userid = '".$userid."'";
        $sth = $pdo->query($query);
        ...
        while($row = $sth->fetch(PDO::FETCH_BOTH)) {
            ...
            echo '<td>' . $row['incamount'] . '</td>';
            echo '<td>' . $row['incdescr'] . '</td>';
            echo '</tr>';
        }
        echo '</tbody>';
        echo '</table>';
        echo '<br/><p><a href="datepickerin.php" class="btn btn-primary">НАЗАД</a>
